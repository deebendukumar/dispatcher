...
import static io.zestic.coin.core.Utils.*;
 ...
 * its contents. See the BitCoin technical paper for more detail on blocks.<p>
 * <p>
 ...
 * {@link Peer#getBlock(byte[])}, or grab one from a downloaded
 * {@link BlockChain}.
 */
public class Block extends Message {
     ...
     * hash.
     */
    static private final BigInteger LARGEST_HASH = BigInteger.ONE.shiftLeft(256);
    ...
    private byte[] calculateHash() {
        try {
            ...
            writeHeader(bos);
            return Utils.reverseBytes(Utils.doubleDigest(bos.toByteArray()));
        } catch (IOException e) {
     ...
     * work is invalid, if the timestamp is too far from what it should be, or
     * if the transactions don't hash to the value in the merkle root field.
     * This is <b>not</b> everything that is required for a block to be valid,
     * only what is checkable independent of the chain.
     *

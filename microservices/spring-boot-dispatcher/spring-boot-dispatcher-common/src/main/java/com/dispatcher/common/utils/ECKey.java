...
import io.zestic.coin.bouncycastle.asn1.x9.X9ECParameters;
import io.zestic.coin.bouncycastle.crypto.AsymmetricCipherKeyPair;
import io.zestic.coin.bouncycastle.crypto.generators.ECKeyPairGenerator;
...
import io.zestic.coin.bouncycastle.crypto.signers.ECDSASigner;
...
/**
 * Represents an elliptic curve keypair that we own and can use for signing
 * transactions. Currently, Bouncy Castle is used. In future this may become an
 * interface with multiple implementations using different crypto libraries. The
 * class also provides a static method that can verify a signature with just the
 * public key.<p>
 */
public class ECKey implements Serializable {
    ...
    /**
     * Creates an ECKey given only the private key. This works because EC public
     * keys are derivable from their private keys by doing a multiply with the
     * generator value.
     */
    ...
    /**
     * Calcuates an ECDSA signature in DER format for the given input hash. Note
     * that the input is expected to be 32 bytes long.
     */
